...
::: {.callout-note}
Until now I couldn't manage to replicate @fig-05-04.
:::

```{r}
#| label: repl-5-4
#| attr-source: '#lst-repl-5-3 lst-cap="Replication of Figure 5.4: The beta distribution for getting the card we are interested in, given our data"'
#| include: false

p <- integrate(function(x) dbeta(x,5,1195),0.005,1)

d <- tibble::tibble(x = seq(from = 0, to = 1, by = 0.01),
#               y = dbeta(x, 0.005, 1), 5, 1195) |> 
#               y = pbinom(dbeta(x, 5, 1195), 1200, 0.005)) |> 
#               y = dbeta(pbinom(x, 1200, .005), 5, 1995))
#               y = pbeta(dbeta(x, 5, 1195), 1200, 0.005))
#               y = pbinom(dbeta(x, 5, 1195), 1200, 0.005, lower.tail = FALSE))
#               y = dbeta(pbinom(x, 1200, .005, lower.tail = FALSE), 5, 1995))
#               y = dbeta(pbinom(x, 1200, .005, lower.tail = TRUE), 5, 1995))
#               y = dbeta(x, 0.005, 1), 5, 1195, lower.tail = TRUE)
#               y = pbeta(x, 0.005, 1), 5, 1195, lower.tail = TRUE)
#               y = pbinom(0, 1200, dbeta(x, 5, 1195), lower.tail = FALSE))
#               y = dbeta(pbinom(0, 1200, 0.005, lower.tail = FALSE), 5, 1195))
#               y = dbeta(dbinom(0, 1200, 0.005), 5, 1195)
#               y = pbinom(dbeta(x,5,1195),0.005,1, lower.tail = FALSE)
#               y = pbeta(dbeta(x,5,1195),0.2850559,1, lower.tail = FALSE)
               y = dbeta(pbeta(x, 0.005, 1, lower.tail = FALSE), 5, 1195) # !!
